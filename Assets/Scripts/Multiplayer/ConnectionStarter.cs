...
public class ConnectionStarter : MonoBehaviour
{
    ...
    public HeadlessStartMode serverBuildStartMode =
        HeadlessStartMode.None;
    ...
    public void GotoScene()
    {
        if (goToSceneName is "")
        {
            Debug.LogWarning("goToSceneName is empty");
            return;
        }
        if (!SceneExists(goToSceneName))
        {
            Debug.LogWarning("goToSceneName is not exits");
            return;
        }
    ...
    private void Start()
    {
        ...
        else if (serverBuildStartMode == HeadlessStartMode.AutoStartHost)
        {
            InstanceFinder.ServerManager.StartConnection();
            InstanceFinder.ClientManager.StartConnection();
            GotoScene();
        }
        else if (serverBuildStartMode == HeadlessStartMode.None)
            showGui = true;
    }
